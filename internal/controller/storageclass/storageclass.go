...
var (
	...
	_ ctrl.ContollerManager = &ReconcileStorageClass{}
)
...
func (r *ReconcileStorageClass) Reconcile(ctx context.Context,
	...
	util.UsefulLog(ctx, "req: %v", request.NamespacedName)
	...
	if err != nil {
		util.ErrorLogMsg("failed to get credentials from secret %v", err)
		return reconcile.Result{}, err
	}
	defer cr.DeleteCredentials()
	monitors, _, err := util.GetMonsAndClusterID(ctx, r.config.ClusterId, false)
	if err != nil {
		util.ErrorLog(ctx, "failed getting mons (%v)", err)
		return reconcile.Result{}, err
	}
	...
	if err != nil {
		util.ErrorLog(ctx, "cron.Parse failed: %v", err)
		return reconcile.Result{}, err
	}
	if !ok {
		util.UsefulLog(ctx, "add trash schedule for %s, %s", poolName, r.config.ScheduleSpec)
		loc := time.Now().Location()
		next = schedule.Next(time.Now().In(loc))
		r.poolMap[poolName] = next
	}

	util.UsefulLog(ctx, "next trash schedule for %s, %v", poolName, next)

	if next.Before(time.Now()) {
		util.UsefulLog(context.TODO(), "start trash purge %s %s", poolName, monitors)
		args := buildTrashPurgeArgs(poolName, monitors, cr)
		util.UsefulLog(context.TODO(), "trash purge: %v", args)
